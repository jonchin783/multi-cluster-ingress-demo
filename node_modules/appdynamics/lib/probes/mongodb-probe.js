...
MongodbProbe.prototype.attach = function(obj) {
  ...
  self.agent.on('destroy', function() {
    ...
    proxy.release(obj.Db.prototype._executeQueryCommand);
    proxy.release(obj.Db.prototype._executeInsertCommand);
    proxy.release(obj.Db.prototype._executeUpdateCommand);
    proxy.release(obj.Db.prototype._executeRemoveCommand);
  ...
  if ('instrument' in obj) {
    ...
    collectionCommands.forEach(function(command) {
      proxy.around(obj.Db.prototype, command, function(obj, args, locals) {
        var commandDetails;
        locals.driver = '1.x';

        var serverPool = self.getServerPool(obj);
        var category;
        ...
        if(command && command.db) {
          ...
          if (serverPool.length) {
            ...
            if(command.db.auths && command.db.auths.length > 0) {
              commandDetails.auth = command.db.auths[0];
            }
